          ...
          <div class="card">
            ...
            <div class="card-body table-responsive p-0">
              <table class="table table-striped table-valign-middle">
                ...
                <tbody>
                  <?php
                  $owlist = $con->query("SELECT u.name, p.id as psid, u.id as uid, p.title as psname FROM worker_assign w JOIN processing_steps p ON p.id=w.processing_steps_id JOIN users u ON u.id=w.user_id GROUP BY u.id;")->fetch_all(MYSQLI_ASSOC);
                  $a = array();
                  foreach ($owlist as $s) {
                  ?>
                    <tr>
