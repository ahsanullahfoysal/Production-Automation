...
$d = $con->query("SELECT p.date,p.id,p.invoice_id as invoice,p.price,p.quantity,m.name as material,s.company_name as supplier FROM purchase p JOIN raw_materials m ON p.material_id=m.id JOIN suppliers s ON p.supplier_id=s.id WHERE p.invoice_id=$invid;")->fetch_all(MYSQLI_ASSOC);
$gtotal = 0;
?>
